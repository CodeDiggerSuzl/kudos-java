 ...
 * 比如PowerMockito.when(mock实例，"私有方法名"，anyInt()).thenReturn(期望返回值)。
 * 上面例子中进行断言时，调用私有方法采取了调用公共方法来间接调用私有方法的形式，
 * 单元测试代码对业务代码造成了入侵，因此如果仅仅只是为了验证一个私有方法，可以使用 Whitebox 来方便的调用私有方法:
 * </p>
 *
 * @author suzailong
